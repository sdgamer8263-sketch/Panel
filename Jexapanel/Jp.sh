#!/bin/bash
# Jexactyl Auto Installer with OS Detection + SDGAMER Banner
set -e
...
exit_and_redirect() {
    echo -e "\n${MAGENTA}👋 Installation summary finished.${NC}"
...
echo -e ""
echo -e "${PURPLE}${BOLD}Starting SDGAMER Deployment...${NC}"
sleep 2
...
animate_text "Deployment: PHP 8.3, MariaDB, Nginx, Redis, and extensions..."
...
run_silent "Installing Composer" "curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer"
...
cd /var/www/jexactyl
run_silent "Downloading Jexactyl" "curl -Lo panel.tar.gz https://github.com/jexactyl/jexactyl/releases/download/${JEXACTYL_VERSION}/panel.tar.gz"
run_silent "Extracting files" "tar -xzf panel.tar.gz"
run_silent "Setting permissions" "chmod -R 755 storage/* bootstrap/cache/ && chown -R www-data:www-data /var/www/jexactyl/*"

# Step 4: Database Setup
echo -e "\n${PURPLE}${BOLD}╔════════ Step 4: Database Setup ════════╗${NC}"
run_silent "Starting MariaDB" "systemctl enable --now mariadb"
...
run_silent "Copying .env" "cp -R .env.example .env"
...
run_silent "Application key" "php artisan key:generate --force"
run_silent "Database migration" "php artisan migrate --seed --force"

# Step 6: Service Config
echo -e "\n${PURPLE}${BOLD}╔════════ Step 6: Service Configuration ════════╗${NC}"
run_silent "Setting up cron" "systemctl enable --now cron"
...
mkdir -p /etc/certs/SDG
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 -subj "/C=NA/ST=NA/L=NA/O=SDGAMER/CN=${DOMAIN}" -keyout /etc/certs/SDG/privkey.pem -out /etc/certs/SDG/fullchain.pem > /dev/null 2>&1
...
run_silent "Finalizing" "sed -i '/^APP_ENVIRONMENT_ONLY=/d' .env && echo 'APP_ENVIRONMENT_ONLY=false' >> .env"
